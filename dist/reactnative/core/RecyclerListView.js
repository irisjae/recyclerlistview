...
var RecyclerListView = /** @class */ (function (_super) {
    ...
    function RecyclerListView(props, context) {
        ...
        _this._pendingForceLayout = false;
        _this._baseAutoLayoutId = 0x00000000;
        _this._autoLayoutId = 0x00000000;
        _this.onVisibleIndicesChanged = null;
        ...
        _this.scrollToOffset = function (x, y, animate, useWindowCorrection, relativeIndex) {
                ...
                if (relativeIndex > -1) {
                    ...
                    if (preserveVisiblePosition && layoutManager_1) {
                        ...
                        if (_this._autoLayout) {
                            ...
                            _this._innerScrollComponent.setNativeProps({ preservedIndex: relativeIndex, autoLayoutId: _this._autoLayoutId });
                        }
                        if (animate) {
        ...
        _this._onVisibleIndicesChanged = function (all, now, notNow) {
            if (_this.onVisibleIndicesChanged) {
                _this.onVisibleIndicesChanged(all, now, notNow);
            }
        };
        ...
        _this._virtualRenderer.attachVisibleItemsListener(_this._onVisibleIndicesChanged);
    ...
    RecyclerListView.prototype.componentDidUpdate = function () {
        ...
        if (this._pendingForceLayout) {
            setTimeout(function () {
                _this._pendingForceLayout = false;
                var layoutManager = _this._virtualRenderer.getLayoutManager();
                var _innerScrollComponent = _this._innerScrollComponent;
                if (layoutManager && _innerScrollComponent) {
                    var preservedIndex = layoutManager.preservedIndex();
                    var layouts = layoutManager.getLayouts();
                    if (preservedIndex > -1) {
                        var i = 0;
                        for (var key in _this.state.renderStack) {
                            if (_this.state.renderStack.hasOwnProperty(key)) {
                                if (_this.state.renderStack[key].dataIndex === preservedIndex) {
                                    // @ts-ignore
